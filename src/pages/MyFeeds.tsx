    ...
    try {
      ...
      if (teamInterests && teamInterests.length > 0) {
        ...
        const { data: teams } = await supabase
          .from("teams")
          .select("id, display_name, city_state_name")
          .in("id", teamIds);
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                  {selectedTopics.map((topic) => (
                    ...
                    >
                      ...
                      <div className="text-sm text-muted-foreground">{topic.sport}</div>
                    </div>
                  ))}
                </div>
              )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                  {selectedTeams.map((team) => (
                    ...
                    >
                      ...
                      <div className="text-sm text-muted-foreground">{team.city_state_name}</div>
                    </div>
                  ))}
                </div>
              )}
