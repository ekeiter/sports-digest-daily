...
CREATE OR REPLACE FUNCTION public.get_trending_teams(p_hours integer DEFAULT 2, p_limit integer DEFAULT 20)
 RETURNS TABLE(entity_type text, entity_id bigint, entity_name text, article_count integer, logo_url text, league_code text, league_logo_url text, league_id bigint)
 LANGUAGE sql
...
AS $function$
  ...
  school_counts AS (
    SELECT 'school'::text AS entity_type, aschm.school_id::bigint AS entity_id, NULL::bigint AS league_id,
  ...
  SELECT
    ...
    CASE 
      ...
      WHEN r.entity_type = 'school' THEN (
        SELECT l.code FROM public.league_schools ls JOIN public.leagues l ON l.id = ls.league_id
        WHERE ls.school_id = r.entity_id::integer AND ls.is_active = true ORDER BY l.app_order_id NULLS LAST LIMIT 1)
    ...
    CASE 
      ...
      WHEN r.entity_type = 'school' THEN (
        ...
        JOIN public.leagues l ON l.id = ls.league_id LEFT JOIN league_logos pmi ON pmi.entity_id = l.id
        WHERE ls.school_id = r.entity_id::integer AND ls.is_active = true ORDER BY l.app_order_id NULLS LAST LIMIT 1)
      WHEN r.entity_type = 'country' THEN (
