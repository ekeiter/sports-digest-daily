...
-- Drop the old 3-param version that was just created (not the active one)
DROP FUNCTION IF EXISTS public.get_subscriber_feed(uuid, integer, integer);

-- Update the active 8-param version's matched_interests for schools and countries
...
BEGIN
  ...
  -- Schools WITH league: "school:ShortName:LEAGUE_CODE:league_logo_url" with pmi fallback
  INSERT INTO temp_matched (article_id, label)
  ...
  -- Schools WITHOUT league: "school:ShortName::" for "- All" display
  INSERT INTO temp_matched (article_id, label)
  ...
  -- Country + League (non-Olympics): "country:LeagueName:flag_url"
  INSERT INTO temp_matched (article_id, label)
