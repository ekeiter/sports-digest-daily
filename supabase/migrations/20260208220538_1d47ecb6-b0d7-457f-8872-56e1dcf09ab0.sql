...
-- Fix bug in get_subscriber_feed: Country+League label was generated without verifying 
-- the article actually has the league in article_league_map.
-- This caused Olympics articles with USA country to incorrectly get tagged with "FIFAM - United States"
...
BEGIN
  ...
  -- FIX: Country + League (non-Olympics) - now verifies article also has the league
  INSERT INTO temp_matched (article_id, label)
