-- Update get_subscriber_feed to include sports filtering
    ...
    and (
      ...
      exists (
        ...
        where si.subscriber_id = p_subscriber_id
          and si.kind = 'topic'
          and exists (
            select 1 from public.article_topic_map atp
            where atp.article_id = a.id and atp.topic_id = si.subject_id
          )
      )
      or
      exists (
        select 1
        from public.subscriber_interests si
        join public.sports s on s.id = si.subject_id
        where si.subscriber_id = p_subscriber_id
          and si.kind = 'sport'
          and exists (
            select 1 from public.article_sport_map asm
            where asm.article_id = a.id and asm.sport = s.sport
          )
      )
    )
    and (
