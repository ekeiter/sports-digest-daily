-- Drop existing function to avoid overload ambiguity
DROP FUNCTION IF EXISTS public.get_subscriber_feed(uuid, integer, timestamp with time zone, bigint, bigint, text, bigint, bigint);

-- Recreate with sport expansion for parent-child relationships
CREATE OR REPLACE FUNCTION public.get_subscriber_feed(p_subscriber_id uuid, p_limit integer DEFAULT 100, p_cursor_time timestamp with time zone DEFAULT NULL::timestamp with time zone, p_cursor_id bigint DEFAULT NULL::bigint, p_interest_id bigint DEFAULT NULL::bigint, p_entity_type text DEFAULT NULL::text, p_entity_id bigint DEFAULT NULL::bigint, p_focus_league_id bigint DEFAULT NULL::bigint)
...
BEGIN
  ...
  IF p_entity_type IS NOT NULL AND p_entity_id IS NOT NULL THEN
    -- Expand sport_id to include children if entity is a sport
    IF p_entity_type = 'sport' THEN
      v_sport_ids := get_sport_with_children(p_entity_id::int);
    END IF;
    ...
    WITH matching_articles AS (
      ...
      UNION
      -- Sport with child expansion
      SELECT DISTINCT asm.article_id FROM article_sport_map asm WHERE p_entity_type = 'sport' AND asm.sport_id = ANY(v_sport_ids)
      UNION
      -- School without league filter (All Sports)
      SELECT DISTINCT ascm.article_id
      FROM article_school_map ascm
      WHERE p_entity_type = 'school'
        AND ascm.school_id = p_entity_id
        AND p_focus_league_id IS NULL
      UNION
      ...
      WHERE p_entity_type = 'school'
        AND ascm.school_id = p_entity_id
        AND p_focus_league_id IS NOT NULL
        AND EXISTS (
          SELECT 1
          FROM article_league_map alm
          WHERE alm.article_id = a.id
            AND alm.league_id = p_focus_league_id
        )
      UNION
      -- Country without league filter
      SELECT DISTINCT acm.article_id
      FROM article_country_map acm
      WHERE p_entity_type = 'country'
        AND acm.country_id = p_entity_id
        AND p_focus_league_id IS NULL
      UNION
      -- Country with league filter (specific league intersection)
      SELECT DISTINCT a.id
      FROM article_country_map acm
  ...
  IF p_interest_id IS NOT NULL THEN
    ...
    -- Expand sport_id to include children if interest has a sport
    IF v_interest.sport_id IS NOT NULL THEN
      v_sport_ids := get_sport_with_children(v_interest.sport_id::int);
    END IF;
    ...
    WITH matching_articles AS (
      ...
      UNION
      -- Sport with child expansion (standalone sport interest)
      SELECT DISTINCT asm.article_id FROM article_sport_map asm WHERE v_interest.sport_id IS NOT NULL AND v_interest.league_id IS NULL AND v_interest.school_id IS NULL AND v_interest.team_id IS NULL AND (v_interest.is_olympics IS NULL OR v_interest.is_olympics = false) AND asm.sport_id = ANY(v_sport_ids)
      UNION
