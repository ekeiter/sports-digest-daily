...
-- Fix get_subscriber_feed to use intersection (AND) logic for ALL compound interests
-- School+League requires both to match
-- Olympics requires is_olympics + sport (if set) + country (if set)
...
BEGIN
  ...
  WITH matching_articles AS (
    ...
    -- Leagues ONLY (no school/team attached - pure league follow)
    SELECT DISTINCT alm.article_id
    FROM subscriber_interests si
    JOIN article_league_map alm ON alm.league_id = si.league_id
    WHERE si.subscriber_id = p_subscriber_id 
      AND si.league_id IS NOT NULL
      AND si.school_id IS NULL  -- No school attached = pure league follow
      AND si.team_id IS NULL    -- No team attached
      AND (si.is_olympics IS NULL OR si.is_olympics = false)
      AND (NOT v_has_focus OR si.is_focused = true)
    ...
    WHERE si.subscriber_id = p_subscriber_id 
      ...
      AND si.league_id IS NULL  -- No league attached = pure sport follow
    ...
    WHERE si.subscriber_id = p_subscriber_id 
      ...
      AND si.league_id IS NULL  -- No league = pure school follow (all sports for this school)
      AND (si.is_olympics IS NULL OR si.is_olympics = false)
      AND (NOT v_has_focus OR si.is_focused = true)
    ...
    -- Countries ONLY (non-Olympics context)
