...
-- Recreate with matched_interests output
...
BEGIN
  ...
  -- CASE 3: Combined feed (all favorites)
  ELSE
    INSERT INTO temp_feed_articles
    SELECT DISTINCT a.id
    FROM articles a
  ...
  -- Return articles with matched interests computation
  RETURN QUERY
  SELECT
    a.id AS article_id,
    a.title,
    ...
    a.published_at,
    a.updated_at,
    -- Compute matched interests for combined feed only (for focused feeds it's implicit)
    CASE 
      WHEN p_entity_type IS NOT NULL OR p_interest_id IS NOT NULL THEN
        NULL::text[]
      ELSE
        (
          ...
          FROM (
            ...
            JOIN article_team_map atm ON atm.article_id = a.id AND atm.team_id = si.team_id
            JOIN teams t ON t.id = si.team_id
            WHERE si.subscriber_id = p_subscriber_id AND si.team_id IS NOT NULL
            ...
            FROM subscriber_interests si
            JOIN article_person_map apm ON apm.article_id = a.id AND apm.person_id = si.person_id
            JOIN people p ON p.id = si.person_id
            WHERE si.subscriber_id = p_subscriber_id AND si.person_id IS NOT NULL
            
            UNION ALL
            
            -- Leagues (standalone, not part of school/country/olympics)
            SELECT l.code AS label
            FROM subscriber_interests si
            JOIN article_league_map alm ON alm.article_id = a.id AND alm.league_id = si.league_id
            ...
            -- Sports (standalone)
            SELECT COALESCE(sp.display_label, sp.sport) AS label
            FROM subscriber_interests si
            JOIN article_sport_map asm ON asm.article_id = a.id AND asm.sport_id = ANY(get_sport_with_children(si.sport_id))
            ...
            -- Schools with league: "School (League)"
            SELECT sch.short_name || ' (' || l.code || ')' AS label
            FROM subscriber_interests si
            JOIN article_school_map aschm ON aschm.article_id = a.id AND aschm.school_id = si.school_id
            JOIN article_league_map alm ON alm.article_id = a.id AND alm.league_id = si.league_id
            ...
            -- Schools without league: "School (All)"
            SELECT sch.short_name || ' (All)' AS label
            FROM subscriber_interests si
            JOIN article_school_map aschm ON aschm.article_id = a.id AND aschm.school_id = si.school_id
            JOIN schools sch ON sch.id = si.school_id
            WHERE si.subscriber_id = p_subscriber_id 
            ...
            -- Countries with league (non-Olympics): "Country (League)"
            SELECT c.name || ' (' || l.code || ')' AS label
            FROM subscriber_interests si
            JOIN article_country_map acm ON acm.article_id = a.id AND acm.country_id = si.country_id
            JOIN article_league_map alm ON alm.article_id = a.id AND alm.league_id = si.league_id
            ...
            -- Olympics general
            SELECT 'Olympics' AS label
            FROM subscriber_interests si
            WHERE si.subscriber_id = p_subscriber_id 
              AND si.is_olympics = true
              AND si.sport_id IS NULL
              AND si.country_id IS NULL
              AND a.is_olympics = true
            
            UNION ALL
            
            -- Olympics with sport only: "Olympics (Sport)"
            SELECT 'Olympics (' || COALESCE(sp.display_label, sp.sport) || ')' AS label
            FROM subscriber_interests si
            JOIN article_sport_map asm ON asm.article_id = a.id AND asm.sport_id = ANY(get_sport_with_children(si.sport_id))
            JOIN sports sp ON sp.id = si.sport_id
            WHERE si.subscriber_id = p_subscriber_id 
            ...
            -- Olympics with country only: "Olympics (Country)"
            SELECT 'Olympics (' || c.name || ')' AS label
            FROM subscriber_interests si
            JOIN article_country_map acm ON acm.article_id = a.id AND acm.country_id = si.country_id
            JOIN countries c ON c.id = si.country_id
            WHERE si.subscriber_id = p_subscriber_id 
            ...
            -- Olympics with sport + country: "Olympics (Sport, Country)"
            SELECT 'Olympics (' || COALESCE(sp.display_label, sp.sport) || ', ' || c.name || ')' AS label
            FROM subscriber_interests si
            JOIN article_sport_map asm ON asm.article_id = a.id AND asm.sport_id = ANY(get_sport_with_children(si.sport_id))
            JOIN article_country_map acm ON acm.article_id = a.id AND acm.country_id = si.country_id
            JOIN sports sp ON sp.id = si.sport_id
    ...
    END AS matched_interests
  FROM articles a
  JOIN temp_feed_articles tfa ON tfa.article_id = a.id
  WHERE a.is_duplicate = false
    AND (p_cursor_time IS NULL OR 
         (COALESCE(a.published_at, a.created_at), a.id) < (p_cursor_time, p_cursor_id))
