...
-- Add updated check constraint with all leagues including MLS
ALTER TABLE public.user_teams ADD CONSTRAINT user_teams_league_check 
  CHECK (league = ANY (ARRAY['MLB'::text, 'NFL'::text, 'NBA'::text, 'NHL'::text, 'WNBA'::text, 'NCAAF'::text, 'NCAAM'::text, 'MLS'::text]));