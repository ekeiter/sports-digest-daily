...
DROP FUNCTION IF EXISTS public.get_subscriber_feed(uuid, integer, timestamp with time zone, bigint, bigint, text, bigint);

-- Recreate with url_domain in return
...
BEGIN
  ...
  IF p_entity_type IS NOT NULL AND p_entity_id IS NOT NULL THEN
    ...
    WITH matching_articles AS (
      ...
      UNION
      SELECT DISTINCT ascm.article_id FROM article_school_map ascm WHERE p_entity_type = 'school' AND ascm.school_id = p_entity_id
      UNION
