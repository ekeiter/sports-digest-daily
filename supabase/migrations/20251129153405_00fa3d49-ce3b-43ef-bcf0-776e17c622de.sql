-- Drop the old function signatures to avoid overloading conflicts
DROP FUNCTION IF EXISTS public.get_subscriber_feed(uuid, integer, timestamp with time zone, bigint);
DROP FUNCTION IF EXISTS public.get_subscriber_feed(uuid, timestamp with time zone, integer, timestamp with time zone, bigint);

-- Create the correct function signature without p_since
