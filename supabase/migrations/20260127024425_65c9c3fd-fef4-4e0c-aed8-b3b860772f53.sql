...
BEGIN
  ...
  IF p_entity_type IS NOT NULL AND p_entity_id IS NOT NULL THEN
    ...
    WITH matching_articles AS (
      ...
      WHERE p_entity_type = 'team' AND atm.team_id = p_entity_id
      ...
      WHERE p_entity_type = 'person' AND apm.person_id = p_entity_id
      ...
      WHERE p_entity_type = 'league' AND alm.league_id = p_entity_id
      ...
      WHERE p_entity_type = 'sport' AND asm.sport_id = p_entity_id
      
      UNION
      
      -- School focus (all sports for that school)
      SELECT DISTINCT ascm.article_id
      FROM article_school_map ascm
      WHERE p_entity_type = 'school' AND ascm.school_id = p_entity_id
      ...
      WHERE p_entity_type = 'country' AND acm.country_id = p_entity_id
    )
    SELECT 
      a.id AS article_id,
      a.title,
  ...
  -- CASE 2: Interest-based focus (existing favorited item)
