-- Fix the type mismatch: cast article_id to integer in temp tables
DROP FUNCTION IF EXISTS get_subscriber_feed(uuid, integer, timestamptz, integer, integer, text, integer, integer);
...
BEGIN
  -- Create temp tables
  CREATE TEMP TABLE temp_feed_articles (article_id integer PRIMARY KEY) ON COMMIT DROP;
  CREATE TEMP TABLE temp_result_articles (
  ...
  CREATE TEMP TABLE temp_matched (
    article_id integer,
    label text
  ) ON COMMIT DROP;

  -- Build prefs CTE based on parameters
  ...
  ELSIF p_entity_type IS NOT NULL AND p_entity_id IS NOT NULL THEN
    -- Direct entity focus (no favorite required)
  ...
  ELSE
    ...
    FROM (
      ...
      SELECT alm.article_id FROM subscriber_interests si
        ...
        AND si.school_id IS NULL AND si.country_id IS NULL
      UNION
      -- Sports (standalone, non-Olympics)
      SELECT asm.article_id FROM subscriber_interests si
        JOIN article_sport_map asm ON si.sport_id = asm.sport_id
        WHERE si.subscriber_id = p_subscriber_id AND (NOT v_has_focus OR si.is_focused = true)
        AND si.league_id IS NULL AND si.is_olympics = false
      ...
      SELECT aschm.article_id FROM subscriber_interests si
        ...
        AND si.school_id IS NOT NULL AND si.league_id IS NULL
      UNION
      -- Country + League (non-Olympics intersection)
      SELECT acm.article_id FROM subscriber_interests si
        JOIN article_country_map acm ON si.country_id = acm.country_id
        JOIN article_league_map alm3 ON acm.article_id = alm3.article_id AND si.league_id = alm3.league_id
        WHERE si.subscriber_id = p_subscriber_id AND (NOT v_has_focus OR si.is_focused = true)
  ...
  SELECT tr.article_id, t.display_name
  ...
  INSERT INTO temp_matched (article_id, label)
  SELECT tr.article_id, COALESCE(sc.short_name, sc.name) || ' (' || l.code || ')'
  FROM temp_result_articles tr
  ...
  LEFT JOIN leagues l ON si.league_id = l.id
  ...
  INSERT INTO temp_matched (article_id, label)
  SELECT tr.article_id, l.code || ' - ' || c.name
  FROM temp_result_articles tr
