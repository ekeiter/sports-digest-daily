...
DROP FUNCTION IF EXISTS public.get_subscriber_feed(uuid, integer, timestamp with time zone, integer, integer, text, integer, integer);

-- Update get_subscriber_feed: use team nickname for matched interest labels
-- Also return structured info for schools (without league code in parens) and countries
...
BEGIN
  ...
  ELSE
    -- Combined feed: all subscriber interests
  ...
  SELECT 
    ...
    a.url,
    a.thumbnail_url,
    COALESCE(a.published_at, a.first_seen_at, a.created_at) as published_effective,
    a.domain,
    a.url_domain
  FROM articles a
  ...
  -- Compute matched interests for result articles only
  -- Teams: use nickname when available, fall back to display_name
  ...
  -- Schools WITH league: label format "school:ShortName:LEAGUE_CODE:league_logo_url"
  INSERT INTO temp_matched (article_id, label)
  SELECT tr.article_id, 'school:' || COALESCE(sc.short_name, sc.name) || ':' || l.code || ':' || COALESCE(l.logo_url, '')
  FROM temp_result_articles tr
  ...
  INSERT INTO temp_matched (article_id, label)
  SELECT tr.article_id, COALESCE(sc.short_name, sc.name)
  FROM temp_result_articles tr
  JOIN article_school_map aschm ON tr.article_id = aschm.article_id
  JOIN schools sc ON aschm.school_id = sc.id
  JOIN subscriber_interests si ON si.school_id = sc.id AND si.subscriber_id = p_subscriber_id
  ...
  -- Country + League (non-Olympics): label format "country:LeagueName:country_flag_url"
  INSERT INTO temp_matched (article_id, label)
  SELECT tr.article_id, 'country:' || COALESCE(l.display_label, l.name, l.code) || ' - ' || c.name || ':' || COALESCE(c.logo_url, '')
  FROM temp_result_articles tr
  ...
  SELECT 
    ...
    tr.domain,
    tr.url_domain,
    COALESCE(array_agg(DISTINCT tm.label) FILTER (WHERE tm.label IS NOT NULL), ARRAY[]::text[]) as matched_interests
  FROM temp_result_articles tr
